...
public class FenceLevelGeneration : MonoBehaviour
{
    ...
    private void GenerateMap()
    {
        ...
        if (RandomSeed)
        {
            FenceTileGeneration tg = TilePrefab.GetComponent<FenceTileGeneration>();
            tg.RandomizeLevelSeeds();
        }

        // Selecting one of the internal tiles at random to receive the fence posts
        int selectedZ = Random.Range(1, MapDepthInTiles - 1);
        int selectedX = Random.Range(1, MapWidthInTiles - 1);
        ...
        for (int xTileIndex = 0; xTileIndex < MapWidthInTiles; xTileIndex++)
        {
            for (int zTileIndex = 0; zTileIndex < MapDepthInTiles; zTileIndex++)
            {
                ...
                if (selectedZ == zTileIndex && selectedX == xTileIndex)
                {
                    tile.GetComponent<FenceTileGeneration>().FencedIn = true;
                }
            }
        }
    }
